...
const logger = log4js.getLogger('LoanSrvc');
...
class LoanSrvc {
    ...
    */
    getLoans ( request, response ) {
            ...
            .then( dbo_res => {
                if (  !dbo_res.ok ){
                    throw new Error('getLoans: dbo.dbConnect fails !');
                }
   ...
   getLoanById ( request, response ) {
        ...
        .then( dbo_res => {
            if (  !dbo_res.ok ){
                throw new Error('getLoanByid: dbo.dbConnect fails !');
            }
            return dbo.dbSelect(l_selector);
        })
        .then ( r_data => {
    ...
    createLoan(request, response){
        ...
        .then( dbo_res => {
            if (  !dbo_res.ok ){
                throw new Error('createLoan: dbo.dbConnect fails !');
            }
            return dbo.dbNextId()
        })
        .then( next_res=>{
            var l_id = 'loanreq-' + next_res.uuids[0];
    ...
    deleteLoan  ( request, response ) {
        ...
        .then( dbo_res => {
            if (  !dbo_res.ok ){
                throw new Error('deleteLoan: dbo.dbConnect fails !');
            }
