...
describe('Accounts API', function() {
  ...
  it('Post' + i_url +' Expect create account', function(done){
    //this.skip(); 

    
    var l_req={ acnum: '260010001',   
                ccy: '980',   
                actype: '1',
                acname: 'Новый счета клиента',  
                idcust:   3,
                dtopen:  '2020-07-16',  
                dtclose: null, 
                bal: 0     
    ...
    .end(function(err, res) {
        if (err) {
          ...
          i_ins.push(  lrsp.idacnt  );
          done();
        }    
    });
  });  //it

  it('Get' + i_url +' Expect get accounts', function(done){
      //this.skip(); 
      var l_req={};
      if (test_env_lr) {
        console.log('Запрос: ');
  ...
  it('Get by Id' + i_url +' Expect get account by ID', function(done){
    //this.skip(); 
    var l_req={};
  ...
  it('Delete by Id' + i_url +' Expect delete account by ID', function(done){
    //this.skip(); 
    var l_req={};
